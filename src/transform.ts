 ...
 */
export const resolvePropValues = (content: string) => {
...
export const convertToTailwindClasses = (
	...
	return content.replace(KEY_VALUES_REGEXP, (match, ...rest) => {
		const { key, values } = rest[rest.length - 1] as { key: string; values: string };
		if (!values) {
			return match;
		}
		...
		return generateClassesFromValues(
			...
			(value) => {
				if (typeof replacer === 'undefined') {
					...
					return BASE_REPLACER.replace(/\$prop/g, kebabCaseKey).replace(
						/\$value/g,
						value
					);
				}
